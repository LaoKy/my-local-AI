...
W1 = [[random.gauss(0,0.1) for _ in range(D_in)] for _ in range(D_hid)]
W2 = [[random.gauss(0,0.1) for _ in range(D_hid)] for _ in range(D_out)]

def forward(x):
    h = [silu(sum(W1[i][j]*x[j] for j in range(D_in))) for i in range(D_hid)]
    logits = [sum(W2[i][j]*h[j] for j in range(D_hid)) for i in range(D_out)]
    return logits, h
...
def train_step(x, target):
    ...
    dL_dW2 = [[dL_dlogits[i]*h[j] for j in range(D_hid)] for i in range(D_out)]
    dL_dh  = [sum(dL_dlogits[i]*W2[i][j] for i in range(D_out)) for j in range(D_hid)]

    z1     = [sum(W1[i][j]*x[j] for j in range(D_in)) for i in range(D_hid)]
    dL_dz1 = [silu_backward(z1[i], dL_dh[i]) for i in range(D_hid)]
    dL_dW1 = [[dL_dz1[i]*x[j] for j in range(D_in)] for i in range(D_hid)]
...
for x, target in dataset:
    ...
    print(f"Input={x} pred={pred} target={target} "
          f"{'True' if pred==target else 'False'}")